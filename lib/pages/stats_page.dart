...
class _StatsPageState extends State<StatsPage> {
  ...
  Future<Map<String, dynamic>> _load() async {
    ...
    final weekEnd = DateTime(weekStart.year, weekStart.month, weekStart.day, 23, 59, 59, 999)
        .add(const Duration(days: 6));

    int sumMinutes(List<Map<String, Object?>> rows) {
      var total = 0;
      for (final r in rows) {
    ...
    final focusTodayMin = sumMinutes(todaySessions);
    final focusWeekMin = sumMinutes(weekSessions);
    ...
    for (int i = 6; i >= 0; i--) {
      ...
      final d1 = DateTime(d0.year, d0.month, d0.day, 23, 59, 59, 999);
      final rows = await FocusDao.loadSessionsBetween(d0, d1);
      last7.add(sumMinutes(rows));
    }
  ...
  Widget _focusCard(String title, int minutes, int sessions, int? interrupt) {
    String mm(int x) =>
        '${(x ~/ 60).toString().padLeft(2, '0')}小时${(x % 60).toString().padLeft(2, '0')}分钟';
    return Card(
      child: ListTile(
  ...
  Widget _bar7Days(List<int> mins) {
    ...
    final maxY = (maxV == 0 ? 60 : (maxV * 1.2)).toDouble();
    return AspectRatio(
      aspectRatio: 1.6,
      child: BarChart(
        BarChartData(
    ...
    return Column(
      children: [
        ...
        GridView.count(
          ...
          childAspectRatio: 0.65, // 更高格子，彻底避免溢出
          children: cells,
        ),
      ],
    );
